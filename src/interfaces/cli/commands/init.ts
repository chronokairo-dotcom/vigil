...
export function registerInitCommand(program: Command) {
  program
    .command('init')
    .description('Initialize Vigil in the current project directory')
    ...
    .action(async (opts: { name?: string; description?: string }) => {
      ...
      const project = await projects.create({
        name,
        description: opts.description || `Vigil context for ${name}`,
      });

      // Create .Vigil marker file
      const markerPath = path.join(cwd, '.Vigil');
      fs.writeFileSync(markerPath, JSON.stringify({ projectId: project.id }, null, 2), 'utf-8');

      console.log(`\nVigil initialized for: ${name}`);
