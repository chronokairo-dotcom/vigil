 ...
 * view of all Vigil data.
 *
 * Static resources:
 *   Vigil://projects          — List all projects
 *   Vigil://agent-config      — Agent roles and quality tiers
 *
 * Dynamic resources (templates):
 *   Vigil://project/{id}/summary     — Full project summary
 *   Vigil://project/{id}/context     — All context entries
 *   Vigil://project/{id}/decisions   — All architectural decisions
 *   Vigil://project/{id}/pipelines   — All pipelines
 */
...
export function registerResourceEndpoints(server: any) {
    ...
    server.resource(
        'projects',
        'Vigil://projects',
        { description: 'List of all Vigil projects with IDs, names, and status.' },
        async () => {
    ...
    server.resource(
        'project-summary',
        new ResourceTemplate('Vigil://project/{id}/summary', {
            list: async () => {
                const { projectService } = await services();
                const projects = await projectService.list();
                return {
                    resources: projects.map((p: any) => ({
                        uri: `Vigil://project/${p.id}/summary`,
                        name: `${p.name} — Summary`,
    ...
    server.resource(
        'project-context',
        new ResourceTemplate('Vigil://project/{id}/context', {
            list: async () => {
                const { projectService } = await services();
                const projects = await projectService.list();
                return {
                    resources: projects.map((p: any) => ({
                        uri: `Vigil://project/${p.id}/context`,
                        name: `${p.name} — Context`,
    ...
    server.resource(
        'project-decisions',
        new ResourceTemplate('Vigil://project/{id}/decisions', {
            list: async () => {
                const { projectService } = await services();
                const projects = await projectService.list();
                return {
                    resources: projects.map((p: any) => ({
                        uri: `Vigil://project/${p.id}/decisions`,
    ...
    server.resource(
        'project-pipelines',
        new ResourceTemplate('Vigil://project/{id}/pipelines', {
            list: async () => {
                const { projectService } = await services();
                const projects = await projectService.list();
                return {
                    resources: projects.map((p: any) => ({
                        uri: `Vigil://project/${p.id}/pipelines`,
