...
export function registerEventEndpoints(server: any) {
    server.tool(
        'get_recent_events',
        'Get recent cross-process events from the Vigil event bus. Shows pipeline state changes, task completions, etc. from all connected processes (VS Code, CLI, other AIs).',
        {
    ...
    server.tool(
        'emit_event',
        'Emit a custom event to the Vigil event bus. This notifies all connected processes (VS Code extension, CLI, other AIs) in real-time.',
        {
