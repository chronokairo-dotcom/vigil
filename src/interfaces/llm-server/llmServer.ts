...
function getConfig(): ServerConfig {
    const cfg = vscode.workspace.getConfiguration('vigil.llmServer');
    return {
...
async function startServer(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        httpServer!.once('error', (err: NodeJS.ErrnoException) => {
            const msg =
                err.code === 'EADDRINUSE'
                    ? `Port ${port} is already in use. Change vigil.llmServer.port in settings.`
                    : err.message;
...
export async function activateLlmServer(context: vscode.ExtensionContext): Promise<void> {
    outputChannel = vscode.window.createOutputChannel('Vigil LLM Server');
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('vigil.llmServer.start', async () => {
            try {
                await startServer();
            } catch (err: unknown) {
        ...
        vscode.commands.registerCommand('vigil.llmServer.stop', async () => {
            await stopServer();
        }),

        vscode.commands.registerCommand('vigil.llmServer.status', () => {
            const { host, port } = getConfig();
        ...
        vscode.commands.registerCommand('vigil.llmServer.listModels', async () => {
            ...
            outputChannel!.show();
            vscode.window.showInformationMessage(
                `${models.length} model(s) available. See "Vigil LLM Server" output channel for details.`,
            );
        }),
    );
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration(async (e) => {
            if (
                e.affectsConfiguration('vigil.llmServer.host') ||
                e.affectsConfiguration('vigil.llmServer.port')
            ) {
                if (httpServer?.listening) {
