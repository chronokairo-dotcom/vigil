...
function getPipelinesDir(projectId: string): string {
  const dir = path.join(os.homedir(), '.Vigil', 'pipelines', projectId);
  try {
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  } catch (err) {
    console.error(`[Vigil] Cannot create pipelines dir ${dir}: ${(err as Error).message}`);
  }
  return dir;
}
...
function loadPipeline(projectId: string, pipelineId: string): Pipeline | null {
  ...
  try {
    ...
    console.error(`[Vigil] Failed to parse pipeline ${pipelineId}: ${(err as Error).message}`);
    return null;
  }
}
  ...
  list(projectId: string): Pipeline[] {
    ...
    for (const f of files) {
      try {
        ...
        console.error(`[Vigil] Skipping corrupted pipeline file ${f}: ${(err as Error).message}`);
      }
    }
    return results;
  }
