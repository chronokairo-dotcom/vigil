...
function getConfigPath(): string {
  return path.join(os.homedir(), '.Vigil', 'agent-config.json');
}
...
export function loadAgentConfig(): AgentModelConfig {
  ...
  try {
    ...
    console.error(`[Vigil] Failed to load agent config: ${(err as Error).message}`);
  }
...
function getFailureHistoryPath(): string {
  return path.join(os.homedir(), '.Vigil', 'model-failures.json');
}
...
function getOllamaConfigPath(): string {
  return path.join(os.homedir(), '.Vigil', 'ollama-config.json');
}
